...
const FAQSection = () => {
  ...
  return (
    <section ref={sectionRef} id="faq" className="py-32 bg-gradient-to-br from-secondary/30 via-background to-secondary/20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-8 relative z-10">
        ...
        <div className="text-center mb-24">
          <div className="inline-flex items-center space-x-3 px-6 py-3 bg-primary/10 backdrop-blur-md rounded-full border border-primary/20 mb-8 hover:scale-105 transition-transform duration-300 cursor-pointer group">
            ...
            <span className="text-primary font-medium tracking-wide">HELP & SUPPORT</span>
            <div className="w-2 h-2 bg-primary rounded-full group-hover:animate-pulse"></div>
          </div>
        ...
        <div className="max-w-4xl mx-auto mb-16">
          ...
          <div className="relative mb-8">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 bg-white/50 backdrop-blur-md border border-gray-200/50 rounded-2xl text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all duration-300"
            />
          </div>
        ...
        <div className="max-w-5xl mx-auto">
          {filteredFaqs.length > 0 ? (
            <Accordion type="multiple" className="space-y-6" onValueChange={handleAccordionChange}>
              {filteredFaqs.map((faq, index) => {
                ...
                return (
                  <AccordionItem 
                    key={index} 
                    value={`item-${index}`}
                    className="group bg-card rounded-3xl border border-gray-100 shadow-apple hover:shadow-glow transition-all duration-500 hover:border-primary/20 overflow-hidden"
                  >
                    <AccordionTrigger className="px-8 py-6 text-left hover:no-underline transition-all duration-300 group-hover:bg-primary/5">
                      <div className="flex items-start space-x-4 w-full">
                        ...
                        <div className="flex-1 text-left">
                          <div className="flex items-center space-x-3 mb-2">
                            <span className="text-xl font-semibold text-card-foreground tracking-tight group-hover:text-primary transition-colors duration-300">
                              {faq.question}
                            </span>
                            <span className="px-2 py-1 bg-primary/10 rounded-full text-xs text-primary border border-primary/20">
                              {faq.difficulty}
                            </span>
                          </div>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {faq.tags.map((tag, tagIndex) => (
                            <span key={tagIndex} className="px-3 py-1 bg-primary/10 rounded-full text-xs text-primary border border-primary/20">
                              {tag}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="pt-4 border-t border-gray-200/50">
                          <h4 className="text-sm font-semibold text-primary uppercase tracking-wide mb-3">Related Questions</h4>
                          <div className="space-y-2">
                            {faq.relatedQuestions.map((relatedQ, qIndex) => (
                              <div key={qIndex} className="flex items-center space-x-2 text-sm text-muted-foreground hover:text-primary transition-colors duration-300 cursor-pointer">
        ...
        <div className="text-center mt-24">
          ...
          <p className="text-sm text-muted-foreground mt-6 font-light">
            Our expert team is here to help with any questions about our BIM services
          </p>
        </div>
      </div>
    </section>
